...
def sum_pix_area(integral_img, x, y, width, height, indent=1):
    ...
    if y + indent >= height or x + indent >= width:
        if y + indent >= height and x + indent >= width:
            x = width - 1
            y = height - 1
            sum_ = integral_img[y][x] - integral_img[y - indent - 1][x] - integral_img[y][x - indent - 1] + integral_img[y - indent - 1][x - indent - 1]

        elif y <= indent <= x and x + indent >= width:
            sum_ = integral_img[y + indent][width - 1] - integral_img[y + indent][x - indent - 1]
        elif x <= indent <= y and y + indent >= height:
            sum_ = integral_img[height - 1][x + indent] - integral_img[y - indent - 1][x + indent]

        elif y + indent >= height:
            sum_ = integral_img[height - 1][x + indent] - integral_img[y - indent - 1][x + indent] - integral_img[height - 1][x - indent - 1] + integral_img[y - indent - 1][x - indent - 1]

        elif x + indent >= width:
            sum_ = integral_img[y + indent][width - 1] - integral_img[y - indent - 1][width - 1] - integral_img[y + indent][x - indent - 1] + integral_img[y - indent - 1][x - indent - 1]
        return sum_

    if x > indent and y > indent:
        sum_ = integral_img[y + indent][x + indent] \
